"""
Modal sweep calibration wrapper for Properlytic world model v12_sb (SF²M).
Runs inference using SF2M sampler and computes backtest metrics + PIT calibration.

Usage:
    modal run scripts/inference/eval/sweep_stage1_calibration_sb.py --jurisdiction acs_nationwide --origins 2019,2020,2021
"""
import modal
import os
import sys
...
app = modal.App(f"sweep-calib-sb-{_jur}")
...
'''
    ...
    wandb.init(
        ...
        name=f"{run_group}_o{origin}",
        group=run_group,
        tags=["sweep", _version_tag, jurisdiction, "stage1_calib", "sf2m"],
...
def main(
    ...
    sample_size: int = 20_000,
    scenarios: int = 128,
    origins: str = "2019,2020,2021,2022,2023,2024",
    version_tag: str = "v12_sb",
    panel_gcs_path: str = "",
):
    origin_list = [int(o.strip()) for o in origins.split(",")]
    
    print(f"\U0001f680 Launching SF2M sweep calibration on Modal: {len(origin_list)} origins (sequential for OOT chain)")
    print(f"   Jurisdiction: {jurisdiction}")
    print(f"   Origins: {sorted(origin_list)}")
    
    results = []
    # SF2M: No eta/steps/tau grid (those are DDIM-specific).
    # Run origins sequentially so each can load the previous origin's calibrator.
    for origin in sorted(origin_list):
        res = evaluate_checkpoints.remote(
            jurisdiction, bucket_name, origin, sample_size,
            scenarios, version_tag, 0.0, 32, 1.0,
            panel_gcs_path,
        )
        results.append(res)
