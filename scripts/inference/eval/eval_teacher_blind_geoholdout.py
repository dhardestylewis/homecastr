"""
eval_teacher_blind_geoholdout.py
================================
Corrected geoholdout: evaluates the Eurostat teacher on 20% held-out NUTS 
regions with ALL Eurostat-specific features STRIPPED, simulating a truly
blind non-EU prediction.

Specifically zeroed out for holdout regions:
- hist_y -> flat line at GDP-scaled anchor (no price history)
- geo categorical embedding -> "UNKNOWN" token
- price_eur_per_hectare, nuts_level -> zeroed
"""
...
def main():
    ...
    print(f"[{ts()}] Loading adapted data from {data_path}")
...
'''
    ...
    for origin in origins:    
        ...
        print(f"\n==============================================")
        print(f"[{ts()}] BLIND Geoholdout: Origin {origin}")
        print(f"==============================================")
        ...
        # ============================================================
        # BLIND STRIPPING: Zero out Eurostat-specific features
        # ============================================================
        
        # 1. Flatten hist_y to a constant (global median) — no price history
        global_median_log = float(np.median(ctx["y_anchor"]))
        ctx["hist_y"][:, :] = global_median_log  # flat line, no local trajectory
        
        # 2. Zero out price_eur_per_hectare and nuts_level in cur_num
        #    These are the first few numeric columns that are Eurostat-specific
        #    We keep ONLY the macro_ and wdi_ columns (indices vary, 
        #    but price_eur_per_hectare and nuts_level are typically indices 0-2)
        #    We need to identify which columns are Eurostat-specific vs global
        eurostat_only_cols = {'year', 'price_eur_per_hectare', 'nuts_level', 'is_train'}
        global_cols = {c for c in num_use_local if c.startswith('macro_') or c.startswith('wdi_')}
        
        # Zero out non-global numeric features
        for i, col_name in enumerate(num_use_local):
            if col_name not in global_cols and i < ctx["cur_num"].shape[1]:
                ctx["cur_num"][:, i] = 0.0
        
        # 3. Replace geo categorical with index 0 ("UNKNOWN" effectively)
        #    The first categorical is typically geo
        if ctx["cur_cat"].shape[1] > 0:
            ctx["cur_cat"][:, 0] = 0  # Set geo to index 0 (unknown/padding)
        ...
        print(f"[{ts()}] Built BLIND context for {n_valid:,} regions (hist=flat, geo=unknown, local_feats=zeroed)")
        ...
        print(f"\n[{ts()}] BLIND Metrics for Origin {origin}")
        for h in range(1, MAX_HORIZON + 1):
            ...
            if len(preds) > 10:
                ...
                nz = acts_arr != 0
                if nz.sum() > 0:
                    mdape = float(np.median(np.abs(preds_arr[nz] - acts_arr[nz]) / np.abs(acts_arr[nz]))) * 100
                else:
                    mdape = float('nan')
                ...
                print(f"  H{h} (Yr {eyr}): rho: {rho:+.3f} | MdAE: {mdae:.1f}% | MdAPE: {mdape:.1f}% | Bias: {bias:+.1f}% | Covg: {covg:.1f}% | CRPS: {crps_m:.1f}% (n={len(preds)})")
